...
static const char *ARCH_CHOICES[] = {"i686", NULL};
...
static int menu_select(const char *title, const char *const options[],
                       const char *current)
{
   int idx = 0;
   ...
   while (1)
   {
      ...
      if (ch == KEY_UP && idx > 0)
         idx--;
      else if (ch == KEY_DOWN && options[idx + 1])
         idx++;
      else if (ch == '\n')
         return idx;
      else if (ch == 'q' || ch == 27)
...
static void edit_config_menu(BuildConfig *cfg)
{
   ...
   while (1)
   {
      ...
      for (int i = 0; fields[i]; ++i)
      {
         if (i == idx) attron(A_REVERSE);
         const char *val = "";
         if (strcmp(fields[i], "config") == 0)
            val = cfg->config;
         else if (strcmp(fields[i], "arch") == 0)
            val = cfg->arch;
         else if (strcmp(fields[i], "imageFS") == 0)
            val = cfg->imageFS;
         else if (strcmp(fields[i], "buildType") == 0)
            val = cfg->buildType;
         else if (strcmp(fields[i], "imageSize") == 0)
            val = cfg->imageSize;
         else if (strcmp(fields[i], "toolchain") == 0)
            val = cfg->toolchain;
         else if (strcmp(fields[i], "outputFile") == 0)
            val = cfg->outputFile;
         else if (strcmp(fields[i], "outputFormat") == 0)
            val = cfg->outputFormat;
         else if (strcmp(fields[i], "kernelName") == 0)
            val = cfg->kernelName;
      ...
      if (ch == KEY_UP && idx > 0)
         idx--;
      else if (ch == KEY_DOWN && fields[idx + 1])
         idx++;
      else if (ch == 'q' || ch == 27)
         return;
      else if (ch == '\n')
      {
         ...
         {
            int sel =
                menu_select("Select filesystem", FS_CHOICES, cfg->imageFS);
            if (sel >= 0) safe_copy(cfg->imageFS, FS_CHOICES[sel]);
         }
         else if (strcmp(field, "buildType") == 0)
         {
            int sel =
                menu_select("Select build type", BUILD_CHOICES, cfg->buildType);
...
static void tui(BuildConfig *cfg)
{
   ...
   const char *menu_items[] = {
       "Edit configuration", "Save", "Save and build (C builder)",
       "Clean (C builder)",  "Exit", NULL};
   int idx = 0;
   int ch;
   ...
   while (1)
   {
      ...
      mvprintw(4, 2, "Current: %s | %s | %s | %s", cfg->config, cfg->arch,
               cfg->imageFS, cfg->buildType);
      ...
      if (ch == KEY_UP && idx > 0)
         idx--;
      else if (ch == KEY_DOWN && menu_items[idx + 1])
         idx++;
      else if (ch == 'q' || ch == 27)
         break;
      else if (ch == '\n')
      {
         ...
         else if (strcmp(item, "Save and build (C builder)") == 0)
         {
            save_config(cfg);
            endwin();
            printf("Running: ./build/builder --build\n");
            int rc = run_command("./build/builder --build");
         ...
         {
            endwin();
            printf("Running: ./build/builder --clean\n");
            int rc = run_command("./build/builder --clean");
