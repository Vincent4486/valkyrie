...
static ArchConfig ARCHES[] = {
	...
	{"x64", "x86_64-linux-musl", "x86_64-linux-musl-", "X64", "-m64"},
	{"aarch64", "aarch64-linux-musl", "aarch64-linux-musl-", "AARCH64", ""},
...
static int resolve_compiler(const BuildConfig *cfg, const ArchConfig *arch, char *out, size_t outSize) {
	char candidate[PATH_MAX];
	int n = snprintf(candidate, sizeof(candidate), "%s/bin/%sgcc", cfg->toolchain, arch->toolPrefix);
...
static int collect_sources_recursive(const char *base, const char *current, StringList *sources) {
	...
	while ((ent = readdir(dir)) != NULL) {
		...
		char path[PATH_MAX];
		if (snprintf(path, sizeof(path), "%s/%s", current, ent->d_name) >= (int)sizeof(path)) continue;
		...
		if (S_ISDIR(st.st_mode)) {
			...
			if (has_suffix(ent->d_name, ".c") || has_suffix(ent->d_name, ".cpp") || has_suffix(ent->d_name, ".S")) {
				char rel[PATH_MAX];
				const char *start = path + strlen(base);
				if (*start == '/') start++;
				if (snprintf(rel, sizeof(rel), "%s", start) >= (int)sizeof(rel)) continue;
...
static const char *config_flags(const BuildConfig *cfg) {
	return strcmp(cfg->config, "release") == 0 ? "-O3 -DRELEASE -s" : "-O0 -DDEBUG -g";
}
...
static int build_objects(const char *compiler,
	                       const char *sourceRoot,
	                       const char *objectRoot,
	...
	for (size_t i = 0; i < sources.len; ++i) {
		char src[PATH_MAX];
		char obj[PATH_MAX];
		if (snprintf(src, sizeof(src), "%s/%s", sourceRoot, sources.items[i]) >= (int)sizeof(src) ||
		    snprintf(obj, sizeof(obj), "%s/%s", objectRoot, sources.items[i]) >= (int)sizeof(obj)) {
			list_free(&sources);
			list_free(objects);
			return 1;
		}
...
static int build_kernel(const BuildConfig *cfg, const ArchConfig *arch, const char *compiler, const char *variantRoot) {
	char linker[PATH_MAX];
	if (snprintf(linker, sizeof(linker), "kernel/arch/%s/boot/linker.ld", cfg->arch) >= (int)sizeof(linker)) return 1;
	...
	if (snprintf(objRoot, sizeof(objRoot), "%s/kernel", variantRoot) >= (int)sizeof(objRoot) ||
	    snprintf(kernelOut, sizeof(kernelOut), "%s/kernel/%s", variantRoot, cfg->kernelName) >= (int)sizeof(kernelOut) ||
	    snprintf(mapOut, sizeof(mapOut), "%s/kernel/core.map", variantRoot) >= (int)sizeof(mapOut)) {
		return 1;
	}
	...
	if (append_str(flags, sizeof(flags), config_flags(cfg)) != 0 ||
	    ...
	    append_str(flags, sizeof(flags), " -ffreestanding -nostdlib -fno-stack-protector -fno-builtin -Wall -Wextra -Ikernel -Iinclude") != 0) {
		return 1;
	}
	...
	if (crtiIdx < 0 || crtnIdx < 0) {
		fprintf(stderr, "Required startup objects crti.o/crtn.o were not produced\n");
		list_free(&objects);
		return 1;
	}
	...
	char query[MAX_CMD];
	if (snprintf(query, sizeof(query), "%s -print-file-name=crtbegin.o", compiler) >= (int)sizeof(query) ||
	    run_capture(query, crtbegin, sizeof(crtbegin)) != 0 ||
	    snprintf(query, sizeof(query), "%s -print-file-name=crtend.o", compiler) >= (int)sizeof(query) ||
	    run_capture(query, crtend, sizeof(crtend)) != 0) {
		fprintf(stderr, "Unable to resolve crtbegin.o/crtend.o from compiler: %s\n", compiler);
		list_free(&objects);
		return 1;
	}

	char objectList[MAX_CMD] = {0};
	if (append_str(objectList, sizeof(objectList), objects.items[crtiIdx]) != 0 ||
	...
	rc = commandf(
		"%s %s -nostdlib -Wl,-T,%s -Wl,-Map=%s -Wl,-z,relro,-z,now -Wl,-z,noexecstack -Wl,--as-needed -Wl,--export-dynamic -o %s %s -lgcc",
		compiler,
		arch->archFlags,
		linker,
		mapOut,
...
static void build_sysroot_flag(const BuildConfig *cfg, const ArchConfig *arch, char *out, size_t outSize) {
	char sysroot[PATH_MAX];
	if (snprintf(sysroot, sizeof(sysroot), "%s/%s/sysroot", cfg->toolchain, arch->targetTriple) < (int)sizeof(sysroot) &&
	    path_exists(sysroot)) {
...
static int build_user_libmath(const BuildConfig *cfg, const ArchConfig *arch, const char *compiler, const char *variantRoot) {
	char outRoot[PATH_MAX];
	if (snprintf(outRoot, sizeof(outRoot), "%s/usr/libmath_build", variantRoot) >= (int)sizeof(outRoot)) return 1;
	...
	if (snprintf(outLib, sizeof(outLib), "%s/libmath.so", outRoot) >= (int)sizeof(outLib)) {
		list_free(&objects);
		return 1;
	}

	rc = commandf("%s %s %s -shared -Wl,-soname,libmath.so -o %s %s", compiler, arch->archFlags, sysrootFlag, outLib, objList);
	list_free(&objects);
	return rc;
}

static int build_user_shell(const BuildConfig *cfg, const ArchConfig *arch, const char *compiler, const char *variantRoot) {
	char outRoot[PATH_MAX];
	if (snprintf(outRoot, sizeof(outRoot), "%s/usr/sh_build", variantRoot) >= (int)sizeof(outRoot)) return 1;
	if (mkdir_p(outRoot) != 0) return 1;
	...
	if (append_str(flags, sizeof(flags), config_flags(cfg)) != 0 ||
	    append_str(flags, sizeof(flags), " ") != 0 ||
	    append_str(flags, sizeof(flags), arch->archFlags) != 0 ||
	    append_str(flags, sizeof(flags), " -Iusr/sh -D_POSIX_C_SOURCE=200809L ") != 0 ||
	    append_str(flags, sizeof(flags), sysrootFlag) != 0) {
		return 1;
	}
	...
	if (snprintf(outBin, sizeof(outBin), "%s/sh", outRoot) >= (int)sizeof(outBin)) {
		list_free(&objects);
		return 1;
	}

	rc = commandf("%s %s %s -o %s %s", compiler, arch->archFlags, sysrootFlag, outBin, objList);
	list_free(&objects);
	return rc;
}

static int build_userspace(const BuildConfig *cfg, const ArchConfig *arch, const char *compiler, const char *variantRoot) {
	int rc = build_user_libmath(cfg, arch, compiler, variantRoot);
	if (rc != 0) return rc;
	return build_user_shell(cfg, arch, compiler, variantRoot);
}

static int remove_cb(const char *path, const struct stat *st, int typeflag, struct FTW *ftwbuf) {
	(void)st;
	(void)typeflag;
	(void)ftwbuf;
	return remove(path);
...
static int action_target(const BuildConfig *cfg, const ArchConfig *arch, const char *target) {
	char imagePath[PATH_MAX];
	if (snprintf(imagePath, sizeof(imagePath), "build/%s_%s/%s_%s_%s.%s",
	             cfg->arch, cfg->config, cfg->outputFile, cfg->config, cfg->arch, cfg->outputFormat) >= (int)sizeof(imagePath)) {
		return 1;
	}

	if (strcmp(target, "run") == 0) {
		return commandf("python3 ./scripts/base/qemu.py -a %s disk %s", cfg->arch, imagePath);
	}
	if (strcmp(target, "debug") == 0) {
		return commandf("python3 ./scripts/base/gdb.py -a %s disk %s", cfg->arch, imagePath);
	}
	if (strcmp(target, "bochs") == 0) {
		return commandf("python3 ./scripts/base/bochs.py disk %s", imagePath);
	}
	if (strcmp(target, "toolchain") == 0) {
		return commandf("python3 ./scripts/base/toolchain.py %s -t %s", cfg->toolchain, arch->targetTriple);
	}
...
static void usage(const char *prog) {
	fprintf(stderr,
	        "Usage: %s [--build] [--clean] [--target run|debug|bochs|toolchain|fformat|deps] [--set key=val ...]\n"
...
int main(int argc, char **argv) {
	...
	if (snprintf(variantRoot, sizeof(variantRoot), "build/%s_%s", cfg.arch, cfg.config) >= (int)sizeof(variantRoot)) {
		return 1;
	}
	...
	if (strcmp(cfg.buildType, "image") == 0) {
		fprintf(stderr, "buildType=image is not yet implemented in C builder. Use buildType=full/kernel/usr for now.\n");
		return 1;
	}
	if (strcmp(cfg.buildType, "full") == 0) {
		...
		if (rc != 0) return rc;
		fprintf(stderr, "Notice: C builder currently compiles kernel + userland; disk image assembly is pending.\n");
		return 0;
	}
