...
The Valkyrie Operating System uses SCons as its build system and requires a cross-toolchain for x86 targets. The build process is designed primarily for Linux hosts, with partial macOS support for the toolchain.

\subsection{Prerequisites}

\subsubsection{Supported Host Environments}
\begin{itemize}
\item \textbf{Primary:} Linux distributions (Debian/Ubuntu, Fedora, Arch, openSUSE, Alpine).
\item \textbf{macOS:} Partial support for building the cross-toolchain.
\end{itemize}
...
On macOS, install GMP/MPFR/MPC libraries with Homebrew:
